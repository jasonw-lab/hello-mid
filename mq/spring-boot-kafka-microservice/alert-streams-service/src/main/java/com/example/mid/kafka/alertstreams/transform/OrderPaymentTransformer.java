 ...
 * - uses a Punctuator (scheduled callback) to scan states and emit alerts for
 *   deadlines (rule A/B)
 *
 * Monitoring / Kafka notes:
 * - All input events are JSON strings; eventType identifies the type.
 * - Alerts are emitted to the topic defined in TopicNames.ALERTS (key=orderId,
 *   value=AlertRaised JSON).
 * - Punctuator frequency is set via punctuateIntervalMs (10s in PoC); shorter
 *   for testing.
 */
